...
def show_result(denom, resultado, score):
    ...
    lcd.draw_string(10, 30, "{} ({}%)".format(resultado, pct), color, lcd.WHITE)
    ...
    def ejecutar_fase_inferencia(classifier_task):
        ...
        print("Clasificacion: {} | Rotado: {} | Conf: {:.0f}%".format(
        denom, rotado, conf_clas * 100))
        ...
        for label, x1, y1, x2, y2, conf in detections:
            print("  {}: ({},{})-({},{}) conf={:.0f}%".format(
            label, x1, y1, x2, y2, conf * 100))
